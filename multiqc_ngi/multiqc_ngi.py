...
class ngi_metadata:
    def __init__(self):
        ...
        try:
            ...
            self.test_data = None
            if (
                "test_database" in config.kwargs
                and config.kwargs["test_database"] is not None
            ):
                log.info(
                    f"Using test data instead of connecting to StatusDB: {config.kwargs['test_database']}"
                )
            ...
            if self.couch is not None or self.test_data is not None:
                ...
                if "project" in config.kwargs and config.kwargs["project"] is not None:
                    log.info(
                        f"Using supplied NGI project id: {config.kwargs['project']}"
                    )
                ...
                if len(pids) == 1:
                    ...
                    if config.push_statusdb:
                        ...
                        log.info(
                            "Not pushing results to StatusDB. To do this, use --push or set config push_statusdb: True"
                        )
    ...
    def get_ngi_project_metadata(self, pid):
        ...
        if self.test_data is not None:
            ...
            if self.couch is None:
                return None
            try:
                p_view = self.couch.post_view(
                    db="projects", ddoc="project", view="summary"
                ).get_result()
        ...
        if config.analysis_dir and (
            "qc_ngi" in str(config.analysis_dir[0]) or "qc_ngi" in os.listdir()
        ):
            infix = "qc"
        else:
            infix = "pipeline"
        ...
        config.output_fn_name = (
            f"{p_summary['project_name']}_{infix}_{config.output_fn_name}"
        )
    ...
    def get_ngi_samples_metadata(self, pid, s_names=None):
        ...
        if self.test_data is not None:
            ...
            p_view_results = self.couch.post_view_queries(
                db="projects",
                ddoc="project",
                queries=[cloudant_v1.ViewQuery(key=pid)],
                view="samples",
            ).get_result()
            if not len(p_view_results["results"][0]["rows"]) == 1:
                log.error(
                    f"statusdb returned {len(p_view_results['results'][0]['rows'])} rows when querying {pid}"
                )
            else:
                if "sample_meta" not in report.ngi:
                    report.ngi["sample_meta"] = dict()
                report.ngi["sample_meta"].update(
                    p_view_results["results"][0]["rows"][0]["value"]
                )
    ...
    def general_stats_sample_meta(self):
        ...
        if meta is not None and len(meta) > 0:
            ...
            for sid in meta:
                ...
                if s_name is None:
                    log.debug(
                        f"Skipping StatusDB metadata for sample {sid} as no bioinfo report logs found."
                    )
                    continue
                ...
                # NGI name
                try:
                    gsdata[s_name]["user_sample_name"] = report.ngi["ngi_names"][
                        ngi_ids[s_name]
                    ]
                except KeyError:
                    pass
                    ...
                    try:
                        if len(meta[sid]["library_prep"][lp]["sample_run_metrics"]) > 0:
                            if seq_lp is None:
                                ...
                                seq_lp = None
                                log.warn(
                                    f"Found multiple sequenced lib preps for {sid} - skipping metadata"
                                )
                                break
                    except KeyError:
                        pass
                if seq_lp is not None:
                    try:
                        if (
                            meta[sid]["library_prep"][lp]["amount_taken_(ng)"]
                            is not None
                        ):
                            gsdata[s_name]["amount_taken"] = meta[sid]["library_prep"][
                                lp
                            ]["amount_taken_(ng)"]
                    except KeyError:
                        pass
                    try:
                        for lv in sorted(
                            meta[sid]["library_prep"][lp]["library_validation"].keys()
                        ):
                            gsdata[s_name]["lp_concentration"] = meta[sid][
                                "library_prep"
                            ][lp]["library_validation"][lv]["concentration"]
                            formats[s_name] = meta[sid]["library_prep"][lp][
                                "library_validation"
                            ][lv]["conc_units"]
                    except KeyError:
                        pass
            ...
            log.info(
                f"Matched meta for {len(s_names)} samples from StatusDB with report sample names"
            )
            if len(s_names) == 0:
                return None
            ...
            if len(formats_set) > 1:
                log.warning(
                    f"Mixture of library_validation concentration units! Found: {', '.join(formats_set)}"
                )
                for s_name in gsdata:
                    try:
                        gsdata[s_name]["lp_concentration"] = (
                            f"{gsdata[s_name]['lp_concentration']} {formats[s_name]}"
                        )
                    except KeyError:
                        pass
    ...
    def push_statusdb_multiqc_data(self):
        ...
        try:
            ...
            if e.message[0] == "bad_request: invalid UTF-8 JSON":
                log.debug(
                    "Error saving to StatusDB: bad_request: invalid UTF-8 JSON, might be NaNs, trying again..."
                )
    ...
    def connect_statusdb(self):
        ...
        try:
            ...
            log.debug(
                f"Could not open the MultiQC_NGI statusdb config file {conf_file}"
            )
            try:
                with open(os.environ["STATUS_DB_CONFIG"], "r") as f:
                    sdb_config = yaml.safe_load(f)
                    log.debug(
                        f"Got MultiQC_NGI statusdb config from $STATUS_DB_CONFIG: {os.environ['STATUS_DB_CONFIG']}"
                    )
            except (KeyError, IOError):
                log.debug(
                    "Could not get the MultiQC_NGI statusdb config file from env STATUS_DB_CONFIG"
                )
        ...
        couch_server = cloudant_v1.CloudantV1(
            authenticator=CouchDbSessionAuthenticator(couch_user, password)
        )
...
class ngi_after_execution_finish:
    def __init__(self):
        ...
        if config.kwargs.get("disable_ngi", False) is True:
            log.debug(
                "Skipping MultiQC_NGI (after execution finish) as 'disable_ngi' was specified"
            )
            return None
        ...
        try:
            ...
            if getattr(config, "save_remote", False) is True:
                ...
                if exit_status != 0:
                    log.error(
                        "Not able to copy report to remote server: Subprocess command failed."
                    )
